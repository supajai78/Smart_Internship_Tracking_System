...
export default function TeacherEvaluations() {
    ...
    useEffect(() => {
        api.get('/teacher/evaluations').then(r => { setData(r.data); setLoading(false); }).catch(() => setLoading(false));
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post(`/teacher/evaluations/${modal.student._id}`, { scores: scoreArray, comment });
            toast.success('บันทึกการประเมินสำเร็จ');
            setModal({ open: false, student: null });
            const res = await api.get('/teacher/evaluations');
            setData(res.data);
        } catch (err) { toast.error(err.response?.data?.message || 'เกิดข้อผิดพลาด'); }
    };

    const hasEval = (studentId) => data.evaluations.some(e => e.student?._id === studentId);
        ...
        <div className="space-y-6 animate-fade-in">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-surface-500 text-sm mt-0.5">ประเมินผลการปฏิบัติงานของนักศึกษา</p>
                </div>
            </div>
                    ...
                    <div key={s._id} className={`bg-white rounded-2xl p-5 border transition-all hover:shadow-lg ${hasEval(s._id) ? 'border-emerald-200' : 'border-surface-100 hover:border-primary-100'}`}>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm text-white"
                                style={{ background: hasEval(s._id) ? '#10b981' : 'linear-gradient(135deg, #6d3ef2, #22d3ee)' }}>
                                {s.firstName?.charAt(0)}
                            </div>
                            <div>
                                <p className="font-medium text-surface-800 text-sm">{s.firstName} {s.lastName}</p>
                                <p className="text-xs text-surface-400">{s.company?.name || '-'}</p>
                            </div>
                        </div>
                        {hasEval(s._id) ? (
            ...
            <Modal isOpen={modal.open} onClose={() => setModal({ open: false, student: null })} title={`⭐ ประเมิน: ${modal.student?.firstName} ${modal.student?.lastName}`} size="lg">
                <form onSubmit={handleSubmit} className="space-y-4">
                    {criteria.map(c => (
                        <div key={c.name} className="p-3 rounded-xl bg-surface-50">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-sm font-bold" style={{ color: '#6d3ef2' }}>{scores[c.name] || 0}/{c.maxScore}</span>
                            </div>
                    ...
                    <div className="p-3 rounded-xl border-2 border-primary-100 text-center" style={{ background: 'rgba(109,62,242,0.04)' }}>
                        <p className="text-sm text-surface-500">คะแนนรวม</p>
                        <p className="text-2xl font-bold" style={{ color: '#6d3ef2' }}>{totalScore}/100</p>
                    </div>
                    <div>
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit" className="px-6 py-2.5 text-white rounded-xl text-sm font-semibold transition-all hover:scale-[1.02]"
                            style={{ background: 'linear-gradient(135deg, #6d3ef2, #22d3ee)', boxShadow: '0 4px 12px rgba(109,62,242,0.25)' }}>บันทึกประเมิน</button>
                    </div>
                </form>
            </Modal>
        </div>
